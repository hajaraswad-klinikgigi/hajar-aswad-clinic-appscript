...
function testAuditFinanceFinalHealth() {
  const result = auditFinanceFinalHealth_();

  Logger.log(JSON.stringify(result, null, 2));

  if (result.success) {
    Logger.log('✅ Finance Final Health Audit: LULUS. Tidak ada error kritis.');
  } else {
    Logger.log(
      '❌ Finance Final Health Audit: GAGAL. Error: ' +
...
function testAuditFinanceFinalHealthErrorsOnly() {
  const result = auditFinanceFinalHealth_();

  Logger.log(JSON.stringify({
    success: result.success,
    summary: result.summary,
    totals_audit: result.totals_audit,
...
function testAuditFinanceFinalHealthWarningsOnly() {
  const result = auditFinanceFinalHealth_();
...
function testHardenFinanceFinalHealth() {
  const result = hardenFinanceFinalHealth_();

  Logger.log(JSON.stringify(result, null, 2));

  if (result.success) {
    Logger.log('✅ Finance Final Hardening: LULUS. Setup dan audit final aman.');
  } else {
    Logger.log(
      '❌ Finance Final Hardening: GAGAL. Total issue: ' +
      result.total_issues
    );
  }
...
function testHardenFinanceTextColumnValues() {
  const result = hardenFinanceTextColumnValues_();

  Logger.log(JSON.stringify(result, null, 2));

  if (result.success) {
    Logger.log('✅ Finance text column values berhasil di-hardening.');
  } else {
    Logger.log('❌ Finance text column values hardening menemukan issue.');
  }

  return result;
}

function testFinancePermissionGuardH8() {
  const billing = (getBillingsRaw()[0] || {});
  const billingId = String(billing.billing_id || '').trim();

  const tests = [
    {
      name: 'getFinanceSummary tanpa session',
      res: getFinanceSummary({
        period: 'today'
      })
    },
    {
      name: 'getReceivablesReport tanpa session',
      res: getReceivablesReport({})
    },
    {
      name: 'getBillingList tanpa session',
      res: getBillingList({})
    }
  ];
  ...
  if (billingId) {
    tests.push({
      name: 'getBillingById tanpa session',
      res: getBillingById({
        billing_id: billingId
      })
    });
    ...
    tests.push({
      name: 'getBillingInvoiceDeliveryInfo tanpa session',
      res: getBillingInvoiceDeliveryInfo({
        billing_id: billingId
      })
    });

    tests.push({
      name: 'getBillingInstallmentPlan tanpa session',
      res: getBillingInstallmentPlan({
        billing_id: billingId
      })
    });

    tests.push({
      name: 'getBillingInstallmentChangePolicy tanpa session',
      res: getBillingInstallmentChangePolicy({
        billing_id: billingId
      })
    });
  }

  const failedOpen = tests.filter(function(test) {
    return test.res && test.res.success === true;
  });

  const result = {
    success: failedOpen.length === 0,
    message: failedOpen.length === 0
      ? 'H8 permission guard aman. Semua read endpoint Finance menolak akses tanpa session.'
      : 'Ada read endpoint Finance yang masih terbuka tanpa session.',
    total_tests: tests.length,
    failed_open_count: failedOpen.length,
    failed_open: failedOpen.map(function(test) {
      return test.name;
    }),
    tests: tests.map(function(test) {
      return {
        name: test.name,
        blocked: !(test.res && test.res.success === true),
        message: test.res && test.res.message ? test.res.message : ''
      };
    })
  };
...
function testFinanceMutationPermissionGuardH8() {
  const tests = [];

  function pushTest(name, fn) {
    let res = null;
    ...
    try {
      ...
      res = {
        success: false,
        message: 'THROWN: ' + (err && err.message ? err.message : err)
      };
    }
    ...
    const blockedBySession =
      ...
      res.success === false &&
      (
        message.indexOf('sesi login') !== -1 ||
        message.indexOf('session') !== -1 ||
        message.indexOf('tidak valid atau sudah kedaluwarsa') !== -1 ||
        message.indexOf('login ulang') !== -1 ||
        message.indexOf('tidak memiliki izin') !== -1
      );

    tests.push({
      name: name,
      success_value: res ? res.success : null,
      message: res && res.message ? res.message : '',
      blocked_by_permission: blockedBySession
    });
  }

  pushTest('addBillingDiscount tanpa session', function() {
    return addBillingDiscount({
      billing_id: '',
      label: '',
      amount: 0,
      reason: ''
  ...
  pushTest('recordBillingPayment tanpa session', function() {
  ...
  if (typeof createOrReplaceBillingInstallmentPlan === 'function') {
    pushTest('createOrReplaceBillingInstallmentPlan tanpa session', function() {
      return createOrReplaceBillingInstallmentPlan({
        billing_id: '',
        actor_role: 'owner',
        installments: []
      });
  ...
  if (typeof clearBillingInstallmentPlan === 'function') {
    pushTest('clearBillingInstallmentPlan tanpa session', function() {
      return clearBillingInstallmentPlan({
        billing_id: '',
        actor_role: 'owner'
      });
    });
  ...
  if (typeof generateBillingInvoicePdf === 'function') {
    pushTest('generateBillingInvoicePdf tanpa session', function() {
  ...
  if (typeof sendBillingInvoiceEmail === 'function') {
    pushTest('sendBillingInvoiceEmail tanpa session', function() {
  ...
  const failed = tests.filter(function(test) {
    return !test.blocked_by_permission;
  });

  const result = {
    success: failed.length === 0,
    message: failed.length === 0
      ? 'H8 mutation permission guard aman. Semua endpoint mutation Finance menolak akses tanpa session.'
      : 'Ada endpoint mutation Finance yang belum menolak akses tanpa session di lapisan permission.',
    total_tests: tests.length,
