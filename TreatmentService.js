...
// test sementara
function testTreatmentServicePhase6GDoctorSourceLog() {
  const result = {
    success: true,
    stage: '6G-TreatmentService-DoctorSource',
    checked_at: typeof nowIso === 'function' ? nowIso() : new Date().toISOString(),
    ui_read_backend_mode: getTreatmentServiceUiReadBackendMode_(),
    ui_read_supabase_test_enabled: typeof repoIsUiReadSupabaseTestEnabled_ === 'function'
      ? repoIsUiReadSupabaseTestEnabled_()
  ...
  try {
    ...
    result.user_count = Array.isArray(users) ? users.length : -1;

    if (!Array.isArray(users)) {
      result.issues.push({ issue: 'USERS_NOT_ARRAY' });
    }

    users.forEach(function(row) {
      const role = String(row.role || '').trim().toLowerCase() || '(blank)';
      const isActive = String(row.is_active || '').trim().toLowerCase() || '(blank)';

      result.role_counts[role] = (result.role_counts[role] || 0) + 1;
      result.active_counts[isActive] = (result.active_counts[isActive] || 0) + 1;
    });

    result.users_preview = users.slice(0, 12).map(function(row) {
      return {
        user_id: row.user_id || '',
    ...
    result.active_doctors_from_service = {
      success: !!(doctorsRes && doctorsRes.success),
      count: doctorsRes && Array.isArray(doctorsRes.data) ? doctorsRes.data.length : -1,
      preview: doctorsRes && Array.isArray(doctorsRes.data) ? doctorsRes.data.slice(0, 10) : []
    };
